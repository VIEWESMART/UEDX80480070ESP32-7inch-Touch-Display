 ...
 *      DEFINES
 *********************/
#define SPI_TRANSACTION_POOL_SIZE 50	/* 同时进行中 DMA 事务的最大数量 */

/* 在排队其他 DMA 事务之前要保留的 DMA 事务。1/10似乎是一个很好的平衡。太多（或全部），会增加延迟. */
...
#else
#define SPI_TRANSACTION_POOL_RESERVE 1	/* 定义最小大小 */
#endif
...
{
    ESP_LOGI(TAG, "添加 SPI 设备");
    ESP_LOGI(TAG, "时钟速度: %dHz, 模式: %d, CS 引脚: %d",
    ...
    spi_device_interface_config_t devcfg={
        ...
        .flags = SPI_DEVICE_NO_DUMMY | SPI_DEVICE_HALFDUPLEX,	/* 虚拟位应根据需要通过DISP_SPI_VARIABLE_DUMMY显式处理 */
#else
	...
	/* 创建事务池并用 PTR 填充它以spi_transaction_ext_t重用 */
...
void disp_spi_change_device_speed(int clock_speed_hz)
{
    ...
    ESP_LOGI(TAG, "更改 SPI 器件时钟速度: %d", clock_speed_hz);
    ...
    uint64_t addr, uint8_t dummy_bits)
{
    ...
    if (flags & DISP_SPI_RECEIVE) {
		...
		t.base.length = 0;	/* 半双工读取中无 MOSI 相位 */
#else
		t.base.rxlength = 0; /* 在全双工模式下，零表示与 TX 长度相同 */
#endif
    }
    ...
    if (flags & DISP_SPI_SEND_POLLING) {
		...
		/* 如有必要，请确保我们可以通过为一些以前的事务提供服务来排队新事务 */
		if(uxQueueMessagesWaiting(TransactionPool) == 0) {
			...
			while(uxQueueMessagesWaiting(TransactionPool) < SPI_TRANSACTION_POOL_RESERVE) {
				if (spi_device_get_trans_result(spi, &presult, 1) == ESP_OK) {
					xQueueSend(TransactionPool, &presult, portMAX_DELAY);	/* 返回池以重复使用 */
				}
			}
		}
        ...
        if (spi_device_queue_trans(spi, (spi_transaction_t *) pTransaction, portMAX_DELAY) != ESP_OK) {
			xQueueSend(TransactionPool, &pTransaction, portMAX_DELAY);	/* 将失败的事务发送回池以重复使用 */
        }
    }
}
...
void disp_wait_for_pending_transactions(void)
{
	...
	while(uxQueueMessagesWaiting(TransactionPool) < SPI_TRANSACTION_POOL_SIZE) {	/* 服务，直到事务重用池再次填满 */
        if (spi_device_get_trans_result(spi, &presult, 1) == ESP_OK) {

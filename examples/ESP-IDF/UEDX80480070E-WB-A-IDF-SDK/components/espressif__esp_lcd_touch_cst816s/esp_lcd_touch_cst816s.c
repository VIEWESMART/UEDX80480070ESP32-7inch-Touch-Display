...
static esp_err_t read_data(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_read_bytes(tp, 0x10, buf, sizeof(buf)), TAG, "I2C read failed");


    x = ((buf[1] & 0x3F) << 8 | buf[2]);
    y = ((buf[3] & 0x3F) << 8 | buf[4]);

    x=x*800/16384;
    y=y*480/9600;
    point.num =buf[1] >> 7;
    ESP_LOGE(TAG, "x= %d    y =%d    num=%d     !",x,y ,point.num);
    ...
    for (int i = 0; i < point.num; i++) {
        tp->data.coords[i].x =x;// point.x_h << 8 | point.x_l;
        tp->data.coords[i].y =y;// point.y_h << 8 | point.y_l;
    }
